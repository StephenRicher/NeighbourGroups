...
def downsample(
    ...
    df_out = pd.DataFrame(
        {"isolate_id": ids_sub, "target_n": actual_n, "replicate": random_state, "nn_distance": nn_dist}
    )
    logger.info("Writing output to stdout")
    df_out.to_csv(sys.stdout, sep="\t", index=False, header=True)
...
def downsample_cgmlst(
    ...
    kmeans = MiniBatchKMeans(n_clusters=n_clusters, n_init=10, batch_size=2048, random_state=random_state)
